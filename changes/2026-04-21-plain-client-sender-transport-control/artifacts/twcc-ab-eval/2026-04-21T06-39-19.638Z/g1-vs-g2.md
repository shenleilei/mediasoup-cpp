...
| 总体 Loss (%) | 0.00 | 0.00 | - |
| 恢复时间 (ms) | 6400.50 | 6375.00 | -0.40% |
| 稳态抖动 (P90-P50, kbps) | 0.00 | 0.00 | - |
| 队列压力（规范化总和） | - | - | - |
| 多路权重偏差（越小越好） | 0.23 | 0.22 | -1.82% |
...
| AB-003 recovery time not worse, ideally >=15% | not worse, ideally >=15% | 0.40% | `PASS` |
| AB-001 steady-state goodput regression <=5% | >=-5% | 0.00% | `PASS` |
| AB-005 fairness deviation not increased | <=0% | -1.82% | `PASS` |
...
| AB-003 | bw 500kbps / rtt 25ms / loss 0.1 / jitter 5ms | gp=450.00, loss=0.00, rec=6401ms | gp=450.00, loss=0.00, rec=6375ms | 0.00% | `PASS` |
...
| Baseline Loss (mean/p50/p90) | 0.00 / 0.00 / 0.00 % |
| Candidate Loss (mean/p50/p90) | 0.00 / 0.00 / 0.00 % |
| Baseline Recovery (mean/p50/p90) | 6401 / 6454 / 6454 ms |
| Candidate Recovery (mean/p50/p90) | 6375 / 6392 / 6392 ms |
| 队列压力 Baseline/Candidate | - / - |
| 权重偏差 Baseline/Candidate | - / - |
| 变化（goodput/loss/recovery/queue/fairness） | 0.00% / - / -0.40% / - / - |
| 结论 | `PASS` |
...
| 变化（goodput/loss/recovery/queue/fairness） | - / - / - / - / -1.82% |
| 结论 | `PASS` |
...
- 聚合对比 JSON：`changes/2026-04-21-plain-client-sender-transport-control/artifacts/twcc-ab-eval/2026-04-21T06-39-19.638Z/g1-vs-g2.json`
