...
| 角色 | 标签 | 运行模式 | 关键开关 |
|---|---|---|---|
| Baseline | `G1 Controller-Only` | 新发送路径，但关闭 transport estimate | `PLAIN_CLIENT_ENABLE_TRANSPORT_CONTROLLER=1`<br>`PLAIN_CLIENT_ENABLE_TRANSPORT_ESTIMATE=0` |
...
- 这组对比回答：G1 Controller-Only -> G2 Candidate 是否满足当前 TWCC A/B 门槛。
- Baseline 侧重点：作为 controller-only 基线，用来隔离 transport estimate / TWCC 主路径本身的净收益。
