...
// Calibrated against SIGCOMM 2018 / Google GCC empirical data:
// 5% loss → utilization ~0.5, 10% → ~0.25, 20% → ~0.10
...
// RTT amplification: logarithmic model calibrated against TMA 2021 empirical data.
// Low RTT → higher amplification (protocol overhead dominates);
// high RTT → amplification converges toward 1.0.
export function computeReportedRtt(baseRttMs) {
  return Math.round(baseRttMs + 5 + 15 * Math.log2(1 + baseRttMs / 50));
}

// RFC 3550 §6.4.1 jitter smoothing: WebRTC reports EWMA-smoothed jitter
// which is empirically ~75% of raw network jitter due to the α=1/16 filter.
...
export function toSyntheticCondition(network) {
  ...
  // Weights calibrated against SIGCOMM 2018 GCC empirical utilization data.
  // Loss weight raised from 0.15→0.40 to match real GCC aggressiveness.
  let utilization =
    0.98 -
