/**
 * Browser-side harness for downlink QoS v2 signaling verification.
 * Verifies publisher track-scoped overrides generated from subscriber demand.
 */
...
window.__downlinkV2Harness = {
  ...
  async publishPublisherStats(seq = 1) {
    ...
    if (!resp.ok) throw new Error('clientStats failed: ' + JSON.stringify(resp));
    return resp;
  },

  async sendDownlinkStats({
    seq,
    visible = true,
    pinned = false,
