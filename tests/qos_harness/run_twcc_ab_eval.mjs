...
function buildAbScenarioMetrics(result, abId) {
  ...
  return {
    ...
    whiteBox: {
      ...
      probeClusterStarts: phaseCounterDelta(result, config.phase, 'probeClusterStarts'),
      probeClusterCompletes: phaseCounterDelta(result, config.phase, 'probeClusterCompletes'),
      probeClusterEarlyStops: phaseCounterDelta(result, config.phase, 'probeClusterEarlyStops'),
      probeBytesSent: phaseCounterDelta(result, config.phase, 'probeBytesSent'),
...
function renderSummaryReport({
  ...
  const commandArgs = [
    path.join(repoRoot, 'tests', 'qos_harness', 'render_twcc_ab_summary.mjs'),
    `--output=${outputPath}`,
    `--raw-json=${rawJsonPath}`,
  ];

  if (traceDirPath) {
    commandArgs.push(`--trace-dir=${traceDirPath}`);
  }
  for (const pairJsonPath of pairJsonPaths) {
    commandArgs.push(`--pair-json=${pairJsonPath}`);
  }
...
function publishLatestDocsArtifacts({
  ...
  const stableRawGroupsPath = path.join(docsGeneratedDir, 'twcc-ab-raw-groups.json');
  const stableArtifacts = [
    {
      source: rawGroupsPath,
      target: stableRawGroupsPath,
    },
  ];
  ...
  renderSummaryReport({
    ...
    rawJsonPath: rawGroupsPath,
    traceDirPath: path.join(runDirPath, 'raw'),
    pairJsonPaths: pairings.map(pairing => pairing.metricsPath),
  });

  renderSummaryReport({
    outputPath: stableSummaryPath,
    rawJsonPath: stableRawGroupsPath,
    traceDirPath: path.join(runDirPath, 'raw'),
    pairJsonPaths: pairings.map(pairing =>
      path.join(docsGeneratedDir, `twcc-ab-${pairing.name}.json`)
    ),
  });
}
  ...
  for (const pairing of pairings) {
    ...
    for (const abId of selectedAbIds) {
      ...
      const baselineWhiteBox = {
        ...
        probeClusterStarts: aggregateRawMetric(baselineMetrics, item => item?.whiteBox?.probeClusterStarts).mean,
        probeClusterCompletes: aggregateRawMetric(baselineMetrics, item => item?.whiteBox?.probeClusterCompletes).mean,
        probeClusterEarlyStops: aggregateRawMetric(baselineMetrics, item => item?.whiteBox?.probeClusterEarlyStops).mean,
        probeBytesSent: aggregateRawMetric(baselineMetrics, item => item?.whiteBox?.probeBytesSent).mean,
      ...
      const candidateWhiteBox = {
        ...
        probeClusterStarts: aggregateRawMetric(candidateMetrics, item => item?.whiteBox?.probeClusterStarts).mean,
        probeClusterCompletes: aggregateRawMetric(candidateMetrics, item => item?.whiteBox?.probeClusterCompletes).mean,
        probeClusterEarlyStops: aggregateRawMetric(candidateMetrics, item => item?.whiteBox?.probeClusterEarlyStops).mean,
        probeBytesSent: aggregateRawMetric(candidateMetrics, item => item?.whiteBox?.probeBytesSent).mean,
