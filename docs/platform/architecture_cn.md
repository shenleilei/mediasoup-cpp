...
如果只需要构建、测试和常规开发入口，先读 [DEVELOPMENT.md](README.md)。如果需要理解运行时控制流、所有权和时序，再读本文。
如果需要专门看 Linux client，请继续看 [linux-client-architecture_cn.md](../qos/plain-client/architecture_cn.md)。
...
当前主架构文档只保留这一层摘要；完整细节见 [linux-client-architecture_cn.md](../qos/plain-client/architecture_cn.md)。
