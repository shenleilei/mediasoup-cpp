...
截至 `2026-04-16`，仓库内 Linux `PlainTransport C++ client` 的 `uplink QoS` 主链路已经进入可回归状态。
...
- 在 Linux `PlainTransport C++ client` 上已具备对位入口与可追溯结果
...
- 人类可读结果：
  [generated/plain-client-qos-case-results.targeted.md](./generated/plain-client-qos-case-results.targeted.md)
- 结构化 artifact：
