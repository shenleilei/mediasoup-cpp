...
2. server QoS / PlainTransport protocol capability
3. downlink QoS

当前稳定口径：

- browser uplink 主 gate：`55 case`；当前增量验证为原 `43 case` 主 gate `PASS` + `GD1-GD12` targeted `PASS`
- server QoS：C++ QoS 单测、集成测试、录制精度测试和 browser harness 仍是活跃回归入口
- PlainTransport：服务端 `plainPublish` / `plainSubscribe` 能力保留，但根目录原生 WebRTC QoS plain push/play client 已移除
...
- browser uplink 主测试集
- 服务端 QoS 单测、集成测试和 browser harness
- 服务端 PlainTransport 协议能力
- downlink 现有 subscriber-side / publisher-supply 协调能力

### 3.2 不在“当前已完成”口径里的内容

- 根目录原生 WebRTC QoS plain push/play client
- `dynacast`
