...
- 已决定在后续 P1 阶段补一份：
  单次串行 `41 case` 的 full matrix 独立归档结果。

## 建议对上口径

`uplink QoS 当前已经从“局部链路可工作”推进到“现有自动化测试集完成收敛并通过”。后续工作重点不再是修失败 case，而是按既定 P1 计划补一份更适合归档审计的单次 full-matrix 独立产物，并把它固化为长期 gate。`
