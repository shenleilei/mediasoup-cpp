...
| delay/loss/bandwidth/recovery netem 弱网短测 | `scripts/run_webrtc_qos_plain_p2_smoke.sh --enable-netem --cases baseline,delay_100ms,loss_2pct,bandwidth_600k,drop_recover` | 当前主报告：`5 / 5 PASS`，`failedChecks=0`，`weakNetworkCoverage=PASS`；delay RTT avg/max `78.25/220ms`，bandwidth targetBps min/max `300000/2500000`，`drop_recover` targetBps min/avg/max `300000/1398037.20/2500000` |
| recovery first-frame gate | smoke report 对齐 `case_timing.clearEpochMs` 和 QoE `epochMs/decodedFrames`，输出 `recoveryFirstFrame` gate | 当前主报告 `recoveryFirstFrame=PASS`，清网后 `144ms` decoded frames 增长，decoded delta=`376`；专项报告：[generated/webrtc-qos-plain-p2-recovery-first-frame-report.md](./generated/webrtc-qos-plain-p2-recovery-first-frame-report.md) `drop_recover=PASS`、`failedChecks=0`、清网后 `2122ms` 首帧、decoded delta=`416` |
...
- P2 synthetic+QoE+netem smoke 主报告当前为 `PASS`：`baseline`、`delay_100ms`、`loss_2pct`、`bandwidth_600k`、`drop_recover` 均 PASS；`qosMainline=PASS`，`sdkRuntimeObservability=PASS`，`encoderRuntime=PASS`，`nativeDecodeQoe=PASS`，`weakNetworkCoverage=PASS`，`recoveryFirstFrame=PASS`，`failedChecks=0`。
...
| delay netem | PASS | RTT avg/max `78.25/220ms`，`decodeErrors=0` |
| loss netem | PASS | `loss_2pct` 下 `decodeErrors=0`，NACK/RTCP feedback 可观测 |
| bandwidth netem | PASS | `bandwidth_600k` 下 targetBps min/max `300000/2500000`、`decodeErrors=0` |
| recovery control-plane netem | PASS | `drop_recover` 下 targetBps min/avg/max `300000/1398037.20/2500000`，清网后离开最低档；SDK runtime log 记录 `sender_recovery_probe before_target_bps=300000 after_target_bps=1200000 latest_rtt_ms=6 feedback_age_us=0` |
| recovery first-frame gate | PASS | `drop_recover` 清网后 `144ms` decoded frames 增长，decoded delta=`376`，主报告签收 |
...
- browser/真实输入源恢复首帧：native synthetic `drop_recover` 已通过 P2-M8b；browser receiver 和 V4L2 环境后续也要按同一口径扩展。
