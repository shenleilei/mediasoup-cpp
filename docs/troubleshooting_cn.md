...
### 5.5 浏览器房间里看不到 PlainTransport 发布端视频
...
1. 确认 push client 进程还活着，而不是源文件已经播完退出。
2. 确认浏览器加入的是同一个 `roomId`。
3. 如果浏览器是先 join，push client 后 plainPublish，先刷新页面并重新 join 一次。
4. 再去查服务端是否真的完成了 `plainPublish -> auto-subscribe -> newConsumer` 链路。

本次联调里已经确认：

- 现有 `mediasoup-sfu` 不需要为这类 PlainTransport 发布专门重启。
- 问题更常见地出在“浏览器订阅时机”或“push client 已经结束”。
...
./build-webrtc-qos-plain/webrtc-qos-plain-push-client \
  ...
  --room <roomId> \
  --peer plain_push_demo \
  --file /tmp/test_sweep_x20.mp4
```

当 push client 日志已经出现 `plainPublish` 成功时：
