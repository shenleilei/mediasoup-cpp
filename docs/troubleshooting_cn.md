...
### 5.5 PlainTransport 接入方看不到媒体

服务端仍保留 `plainPublish` / `plainSubscribe` 协议能力，但根目录原生 WebRTC QoS plain push/play client 已经移除。排查 PlainTransport 接入问题时，应从外部接入方日志和服务端房间状态入手。
...
1. 确认外部接入方进程还活着，并且仍在向服务端端口发送 RTP/RTCP。
2. 确认浏览器加入的是同一个 `roomId`。
3. 如果浏览器是先 join，外部接入方后 `plainPublish`，先刷新页面并重新 join 一次。
4. 再去查服务端是否真的完成了 `plainPublish -> auto-subscribe -> newConsumer` 链路。

本次联调里已经确认：

- 现有 `mediasoup-sfu` 不需要为这类 PlainTransport 发布专门重启。
- 问题更常见地出在“浏览器订阅时机”或“外部接入方已经停止发送”。
