...
public:
	...
	{
		if (appData_.contains("source") && appData_["source"].is_string()) {
			source_ = appData_["source"].get<std::string>();
		}
	}
	...
	const json& appData() const { return appData_; }
