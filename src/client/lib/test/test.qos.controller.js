...
test('controller completes the stable L1 recovery tail quickly enough to restore L0', async () => {
    const clock = new clock_1.ManualQosClock();
    const healthySamples = [
        createSnapshot(1000, {
            bytesSent: 120000,
    ...
    const statsProvider = {
        getSnapshot: jest
            ...
            .mockResolvedValueOnce(healthySamples[4])
            .mockResolvedValueOnce(healthySamples[5])
            .mockResolvedValueOnce(healthySamples[6])
            .mockResolvedValueOnce(healthySamples[7])
            .mockResolvedValueOnce(healthySamples[8]),
    };
    const actionExecutor = {
        execute: jest
            .fn()
    ...
    expect(controller.level).toBe(0);
    expect(actionExecutor.execute.mock.calls.filter(call => call[0].type === 'setEncodingParameters').map(call => call[0].level)).toEqual([3, 2, 1, 0]);
});
