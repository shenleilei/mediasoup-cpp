...
import { beginProbe, evaluateProbe } from '../qos/probe';
import type { DerivedQosSignals } from '../qos/types';
...
test('beginProbe initializes probe context', () => {
	const probe = beginProbe(3, 2, 1000, false, false);
...
test('probe succeeds after three healthy samples', () => {
	const profile = getDefaultCameraProfile();
	let probe = beginProbe(3, 2, 1000, false, false);

	let evaluation = evaluateProbe(probe, signals(), profile);

	expect(evaluation.result).toBe('inconclusive');
	probe = evaluation.context!;

	evaluation = evaluateProbe(probe, signals(), profile);
	expect(evaluation.result).toBe('inconclusive');
	probe = evaluation.context!;
...
test('probe fails after two bad samples', () => {
	...
	let evaluation = evaluateProbe(probe, badSignals, profile);

	expect(evaluation.result).toBe('inconclusive');
	probe = evaluation.context!;

	evaluation = evaluateProbe(probe, badSignals, profile);
...
test('probe stays inconclusive for mixed samples below completion thresholds', () => {
	...
	expect(evaluation.result).toBe('inconclusive');
	expect(evaluation.context?.healthySamples).toBe(1);
