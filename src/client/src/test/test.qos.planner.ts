import {
	...
	getDefaultCameraProfile,
	getDefaultScreenShareProfile,
} from '../qos/profiles';
import { planActions, planRecovery } from '../qos/planner';
import type { QosPlannerInput } from '../qos/types';

function makeInput(partial: Partial<QosPlannerInput>): QosPlannerInput {
	return {
		source: 'camera',
...
test('planner degrades audio source with bitrate-focused actions', () => {
	const actions = planActions(
		makeInput({
			source: 'audio',
			profile: getDefaultAudioProfile(),
			currentLevel: 1,
			state: 'congested',
		})
	);
...
test('planner pauses screenshare upstream at max level', () => {
	const actions = planActions(
		makeInput({
			source: 'screenShare',
			profile: getDefaultScreenShareProfile(),
			currentLevel: 3,
			state: 'congested',
		})
	);
