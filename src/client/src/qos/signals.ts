...
function asFiniteOrZero(value: number | undefined): number {
...
export function computeBitrateBps(
	currentBytesSent: number | undefined,
	previousBytesSent: number | undefined,
...
export function computeBitrateUtilization(
...
export function classifyReason(
	signals: Pick<
		...
		| 'rttEwma'
	>,
	context: ReasonClassifierContext = {}
): QosReason {
	...
	const networkLikely =
		...
		signals.bitrateUtilization < DEFAULT_NETWORK_WARN_UTILIZATION ||
...
export function deriveSignals(
	current: RawSenderSnapshot,
	previous?: RawSenderSnapshot,
	previousSignals?: DerivedQosSignals,
	options: DeriveSignalsOptions = {}
): DerivedQosSignals {
	const packetsSentDelta = computeCounterDelta(
		current.packetsSent,
		previous?.packetsSent
	);
	const packetsLostDelta = computeCounterDelta(
		current.packetsLost,
		previous?.packetsLost
	);
	const retransmittedPacketsSentDelta = computeCounterDelta(
		current.retransmittedPacketsSent,
		previous?.retransmittedPacketsSent
	);
	const sendBitrateBps = computeBitrateBps(
		current.bytesSent,
		previous?.bytesSent,
		asFiniteOrZero(current.timestampMs),
		previous?.timestampMs
	);
	const targetBitrateBps = Math.max(
		0,
		asFiniteOrZero(current.targetBitrateBps)
	);
	const bitrateUtilization = computeBitrateUtilization(
		sendBitrateBps,
		targetBitrateBps,
		current.configuredBitrateBps
	);
	const lossRate = computeLossRate(packetsSentDelta, packetsLostDelta);
	const lossEwma = computeEwma(
		lossRate,
		previousSignals?.lossEwma,
		options.ewmaAlpha
	);
	const rttMs = Math.max(0, asFiniteOrZero(current.roundTripTimeMs));
	const rttEwma = computeEwma(
		rttMs,
		previousSignals?.rttEwma,
		options.ewmaAlpha
	);
	const jitterMs = Math.max(0, asFiniteOrZero(current.jitterMs));
	const jitterEwma = computeEwma(
		jitterMs,
		previousSignals?.jitterEwma,
		options.ewmaAlpha
	);
