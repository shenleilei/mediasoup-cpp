...
import type { QosQualityLimitationReason, RawSenderSnapshot } from '../types';
import type { ProducerAdapter } from './producerAdapter';

type StatDict = Record<string, unknown>;

function isObject(value: unknown): value is StatDict {
	return typeof value === 'object' && value !== null;
}

function readNumber(obj: StatDict, key: string): number | undefined {
	const value = obj[key];
...
function normalizeTimeToMs(value: number | undefined): number | undefined {
	if (typeof value !== 'number' || !Number.isFinite(value)) {
		return undefined;
	}
...
function readQualityLimitationReason(
	value: unknown
): QosQualityLimitationReason | undefined {
...
function appendQualityDurations(
...
function snapshotFromBase(
	base: Omit<RawSenderSnapshot, 'timestampMs'>
): RawSenderSnapshot {
	return {
		...base,
...
export class ProducerSenderStatsProvider implements QosStatsProvider {
	constructor(private readonly adapter: ProducerAdapter) {}

	async getSnapshot(): Promise<RawSenderSnapshot> {
		const base = this.adapter.getSnapshotBase();
		const report = await this.adapter.getStatsReport();
		const remoteById: Map<string, StatDict> = new Map();
		const outbounds: StatDict[] = [];
		...
		const qualityLimitationDurationsSec: Record<string, number> = {};
		let rttSumMs = 0;
		let rttCount = 0;
		let jitterSumMs = 0;
		let jitterCount = 0;
		...
		for (const outbound of outbounds) {
			...
			frameWidth = Math.max(
				frameWidth,
				readNumber(outbound, 'frameWidth') ?? 0
			);
			frameHeight = Math.max(
				frameHeight,
				readNumber(outbound, 'frameHeight') ?? 0
			);
			framesPerSecond = Math.max(
				framesPerSecond,
				readNumber(outbound, 'framesPerSecond') ?? 0
			);

			if (!qualityLimitationReason) {
				qualityLimitationReason = readQualityLimitationReason(
					outbound.qualityLimitationReason
				);
			}

			appendQualityDurations(
				qualityLimitationDurationsSec,
				outbound.qualityLimitationDurations
			);

			const remoteId =
				typeof outbound.remoteId === 'string' ? outbound.remoteId : undefined;
			...
			const rttMs = normalizeTimeToMs(
				(remote && readNumber(remote, 'roundTripTime')) ??
			...
			const jitterMs = normalizeTimeToMs(
				(remote && readNumber(remote, 'jitter')) ??
		...
		return {
			...
			retransmittedPacketsSent:
				retransmittedPacketsSent > 0 ? retransmittedPacketsSent : undefined,
			...
			qualityLimitationDurationsSec:
				Object.keys(qualityLimitationDurationsSec).length > 0
					? qualityLimitationDurationsSec
					: undefined,
		};
	}
