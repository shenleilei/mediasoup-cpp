...
function resolveMaxLevel(profile: QosProfile): number {
	return Math.max(0, profile.levelCount - 1);
}

function resolveTargetLevel(input: QosPlannerInput): number {
	...
	if (typeof input.overrideClampLevel === 'number') {
		targetLevel = Math.min(
			targetLevel,
			clampLevel(input.overrideClampLevel, maxLevel)
		);
	}
...
function stepToActions(
	step: QosLadderStep,
	source: QosPlannerInput['source'],
	currentLevel: number,
	inAudioOnlyMode: boolean
): PlannedQosAction[] {
	const actions: PlannedQosAction[] = [];
...
export function planActionsForLevel(
	input: QosPlannerInput,
	targetLevel: number
): PlannedQosAction[] {
...
export function planActions(input: QosPlannerInput): PlannedQosAction[] {
	const maxLevel = resolveMaxLevel(input.profile);
	const currentLevel = clampLevel(input.currentLevel, maxLevel);
...
export function planRecovery(input: QosPlannerInput): PlannedQosAction[] {
	return planActions({
		...input,
		state: 'recovering',
	});
