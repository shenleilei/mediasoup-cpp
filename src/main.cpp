...
int main(int argc, char* argv[]) {
	...
	std::string deploymentEnvironment = "development";
	...
	}
	{
		...
		if (cfgFile.is_open()) {
			try {
				...
				if (cfg.contains("environment"))    deploymentEnvironment = cfg["environment"].get<std::string>();
	...
	for (int i = 1; i < argc; i++) {
		...
		else if (arg.find("--env=") == 0)      deploymentEnvironment = arg.substr(6);
	...
	if (signalingPort <= 0 || signalingPort > 65535) return configError("signalingPort must be in range 1-65535");
