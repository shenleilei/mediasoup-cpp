...
### 待实现
1. **[P0] 网络瓶颈突破**: 当前 2 vCPU virtio 单队列环境下，真实网络栈峰值 80 rooms，内核 softirq 是瓶颈（Worker CPU 才 24%）。内核参数调优（netdev_budget/usecs 翻倍）无效。根因是 virtio_net 单队列 NAPI poll 只能单核串行。上线前必须换多队列 virtio VM、物理机 + 万兆网卡、或 SR-IOV
2. **Dynacast**: 按需发送视频层，减少带宽消耗
3. **信令协议标准化**: 定义统一的 request/response/notification schema
4. **多节点录制回放**: 当前每个节点只能查看本地录制文件。方案选项：
