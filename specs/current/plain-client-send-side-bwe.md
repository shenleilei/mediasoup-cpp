...
- 当重传视频（retransmission）与 fresh video 长时间同时排队时，重传**必须**继续保留更高优先级，但**严禁**无限期压制 fresh video；fresh video **必须**保留有界的发送前进机会。
...
- 本规范**未**定义在全局传输带宽上限下、基于权重的完整多轨道媒体调度器（multi-track media scheduler）；除上文明确要求的“音频优先且 fresh video 不得被单独饿死”和“重传优先但 fresh video 不得被无限压制”护栏外，其余多轨道公平性行为仍然在此认可范围之外。
